...
struct enc_info
{
    ...
    /* Both views long-term on disjoint LongTermFrameIdx, each slice naming
       its reference explicitly. See create_encoder. */
    int dual_ltr;
...
/*****************************************************************************/
/* Reference frames to declare: one per view. Dual-LTR declares one more
   than it uses. A client that drops the aux view leaves frame_num gaps,
                   ...
                   int ref_lt_idx, int cur_lt_idx, int send_lt_max)
{
    ...
    else if (is_ltr)
    {
        /* Mark long-term at cur_lt_idx. MMCO 4 raises
           ...
           nothing lowers it again, so dual-LTR sends it once per
                                       ...
                                       struct enc_info **ei)
{
    ...
    /* Reference structure: each AVC444 view predicts from its own previous
       picture. The aux view is a long-term reference, so it survives the
       main pictures of skipped chroma frames; the main view is short-term.
       The default list then has main at index 0 and aux at index 1. */
    /* Dual LTR (default): main on LongTermFrameIdx 0, aux on 1, and every
       ...
       XRDP_AVC444_DUAL_LTR=0 falls back to the single long-term reference
       above. */
    qp_str = g_getenv("XRDP_AVC444_DUAL_LTR");
    lei->dual_ltr = (lei->nviews > 1) &&
                    !(qp_str != NULL && g_atoi(qp_str) == 0);

    LOG(LOG_LEVEL_INFO, "vaapi: encoder %dx%d, %d view(s), QP %d/%d, "
        "deblock %d/%d, dual_ltr %d, rc %s, bitrate %d kbit/s, level %d",
        width, height, lei->nviews, lei->qp_view[0], lei->qp_view[1],
        lei->deblock_view[0], lei->deblock_view[1], lei->dual_ltr,
             ...
             int flags, int defer)
{
    ...
    /* Single LTR: aux long-term, main short-term. Dual LTR: both long-term
       on their own index, the short-term DPB unused. AVC420 is
       short-term. */
    is_ltr = (ei->nviews > 1) && (ei->dual_ltr || (view == 1));
    cur_lt_idx = ei->dual_ltr ? view : 0;
    /* Single LTR sends MMCO 4 every time; dual LTR once per sequence, set
    ...
    /* For a long-term reference frame_idx is LongTermFrameIdx; under
       dual_ltr that is the view. */
    ...
    /* The DPB as the driver models it: short-term references most recent
       first, then long-term. Under dual_ltr only the two long-term ones. */
    nref = 0;
    for (i = 0; i < ei->ndpb; i++)
    {
    ...
    if (ref_lt_idx >= 0)
    {
        /* dual_ltr: one entry, this view's long-term picture (or the IDR),
           named the same way in the packed slice header. */
        slice.RefPicList0[nref++] = ei->ltrv[ref_lt_idx];
    }
    else if (!is_idr)
    {
        /* Default L0 order: short-term by descending PicNum, then
           long-term. */
        for (i = 0; i < ei->ndpb; i++)
        {
    ...
    if (is_ltr)
    {
        ...
        if (ei->dual_ltr)
        {
            ...
            /* An IDR cannot carry MMCO 4 and leaves the ceiling at 0, so
               it must not consume the flag. */
